...
public class UpdateLocaleCommandHandler : IRequestHandler<UpdateLocaleCommand, IdentityModel>
{
    ...
    public async Task<IdentityModel> Handle(UpdateLocaleCommand command, CancellationToken cancellationToken)
    {
        _logger.LogInformation($"Started updating locale for user with [Id] = {command.Id}");
        ...
        _logger.LogInformation($"Finished updating locale for user with [Id] = {command.Id}");
    ...
    private async Task<User> GetUserAsync(string id)
    {
        ...
        _logger.LogError($"Updating locale for user with [Id] = {id} failed. User dow not exist.");
